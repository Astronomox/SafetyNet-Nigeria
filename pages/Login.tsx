  ...
  <label className="block">
    ...
    <div className="relative mt-1.5">
      <input
        ...
        required
        className="w-full border border-line bg-panel-2 px-4 py-3 text-[15px] text-bone placeholder:text-ash/50 focus:border-mint focus:ring-0"
    ...
    <div className="min-h-svh bg-ink text-bone lg:grid lg:grid-cols-2">
      ...
      <AuthPanel className="hidden lg:block lg:min-h-screen">
        <div className="flex h-full flex-col justify-between p-12">
          ...
          <div>
            <p className="eyebrow mb-4 text-mint">9.0820°N · 8.6753°E · NG</p>
            <h1 className="display text-6xl xl:text-7xl">
              Every<br />signal<br /><span className="text-mint">answered.</span>
            </h1>
            <p className="mt-6 max-w-sm text-sm leading-relaxed text-bone/80">
              Real-time incident tracking, AI-verified alerts, and coordinated response across all 36 states.
            </p>
          </div>
        </div>
      </AuthPanel>

      {/* Form column — fills the screen on mobile, no scroll needed */}
      <div className="flex min-h-svh flex-col justify-center px-6 py-6 lg:min-h-screen lg:items-center lg:px-16">
        <div className="w-full max-w-md animate-rise lg:mx-auto">
          {/* Mobile brand header */}
          <Link to={RoutePath.LANDING} className="mb-6 flex items-center gap-3 lg:hidden">
          ...
          <p className="eyebrow text-mint">Operator Access</p>
          <h2 className="display mt-1 text-3xl lg:text-4xl">Sign in</h2>

          {error && (
            <div className="ticks mt-4 border border-flare/40 bg-flare/10 px-4 py-2.5">
          ...
          <form onSubmit={handleEmailLogin} className="mt-5 space-y-4">
            ...
            <div className="flex justify-end">
              <Link to={RoutePath.FORGOT_PASSWORD} className="eyebrow text-[10px] text-mint hover:text-bone">
                Forgot password
              </Link>
            </div>
            <button
          ...
          <p className="mt-5 text-center text-sm text-ash">
            New to the network?{' '}
            <Link to={RoutePath.SIGNUP} className="font-semibold text-mint hover:text-bone">
              Create an account
            </Link>
          </p>
        </div>
